...
- 🔧 **自动优选IP**：支持配置优选IP API，手动或定时自动获取更新优选IP
...
5. 配置自动优选IP（可选）：
   - 点击"自动优选"按钮展开设置
   - **优选IP API地址**：默认 `https://ipdb.api.030101.xyz/?type=bestcf`
   - **自动获取优选IP**：勾选后可通过Cron定时任务自动更新
   - 点击"🔄获取优选IP"可手动立即获取
   - 在Cloudflare Workers后台 → Triggers → Cron Triggers
   - 添加定时任务（例如 `0 */6 * * *` 每6小时执行一次）
